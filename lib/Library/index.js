...
Library.prototype.install = function (name) {
    ...
    if (!lib.unify.exists) {
        this.renderer.warn('Library does not have a unify.json file. [' + name + ']');
        this.client
            .setValue('libs/' + name, {})
...
Library.prototype.uninstall = function (names) {
    var _this = this;
    return this.getAllDependencies([])
        .then(function (all) {
            return _this.getAllDependencies(names)
                .then(function (remaining) {
                    return except(all, remaining);
                });
        })
        .tap(function (remove) {
            remove.forEach(function (lib) {
                this.client
                    .remove('libs/' + lib)
                    .remove('themes/' + lib);
            });
...
Library.prototype.findLibraryDependencies = function (lib) {
    ...
    return getBowerDependencies(bpath);
};
...
Library.prototype.getAllDependencies = function (excluded, create) {
    var bpath = path.join(path.dirname(this.unify.path), 'bower.json');
    if (!fs.existsSync(bpath)) {
        return Promise.resolve([]);
    }
    create = create || this.createDependent.bind(this);
    return getBowerDependencies(bpath)
        .then(function (deps) {
            var flat = [];
            var proms = deps.filter(function (dep) {
                return excluded.indexOf(dep) < 0;
            }).map(function (dep) {
                var lib = create(dep);
                return lib.getAllDependencies(excluded, create)
                    .then(function (subs) {
...
function getBowerDependencies(bpath) {
